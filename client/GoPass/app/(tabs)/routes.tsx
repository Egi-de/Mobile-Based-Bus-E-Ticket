...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, RefreshControl, LayoutAnimation, Platform, UIManager } from 'react-native';
...
export default function RoutesScreen() {
  ...
  const { destination: paramDestination } = useLocalSearchParams<{ destination?: string }>();
  ...
  const fetchRoutes = async () => {
    try {
      ...
      useToastStore.getState().error(getErrorMessage(error), 'Could not load routes');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <ScreenWrapper style={{ backgroundColor: 'transparent' }}>
        <View style={styles.container}>
          ...
          <TouchableOpacity style={styles.header} onPress={toggleExpand} activeOpacity={0.7}>
            ...
            <View style={[styles.chevronContainer, !isExpanded && styles.chevronCollapsed]}>
          ...
          <View style={styles.resultsContainer}>
            <Text style={styles.sectionTitle}>Available Buses ({routes.length})</Text>
            <FlatList
              data={routes}
              ...
              ListEmptyComponent={
                !isLoading ? (
                  <View style={styles.emptyState}>
                    <Ionicons name="bus-outline" size={48} color={theme.colors.text.disabled} />
                    ...
                    {[1, 2, 3].map((i) => (
                      <View key={i} style={styles.skeletonCard}>
                        ...
                        <View style={styles.skeletonBody}>
                          ...
                          <View style={[styles.skeletonBox, { width: '30%', height: 16, marginTop: 8 }]} />
                        </View>
                      </View>
                    ))}
                  </View>
